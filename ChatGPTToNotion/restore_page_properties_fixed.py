...
# -*- coding: utf-8 -*-
"""
Restore Page Properties Fixed - バックアップデータの正しい構造に基づいてプロパティを復元
"""
...
def convert_property_for_update(property_data: Dict[str, Any]) -> Dict[str, Any]:
    """バックアップのプロパティデータをNotion API用に変換"""
    ...
    if prop_type == "url":
        ...
        multi_select_values = property_data.get("multi_select", [])
        if multi_select_values:
            return {
                "multi_select": [
                    {"name": item.get("name", "")} for item in multi_select_values
                ]
            }
        return {"multi_select": []}
    
    elif prop_type == "relation":
        relation_values = property_data.get("relation", [])
        if relation_values:
            return {
                "relation": [
                    {"id": item.get("id", "")} for item in relation_values
                ]
            }
        return {"relation": []}
    
    elif prop_type == "rich_text":
        rich_text_values = property_data.get("rich_text", [])
        if rich_text_values:
            return {
                "rich_text": [
                    {
        ...
        if select_value:
            return {"select": {"name": select_value.get("name", "")}}
        return {"select": None}
        ...
        checkbox_value = property_data.get("checkbox", False)
        return {"checkbox": checkbox_value}
    
    else:
        print(f"未対応のプロパティタイプ: {prop_type}")
        return {}

def main():
    """メイン処理"""
    print("=== ページのプロパティを復元（修正版） ===")
    ...
    for i, page in enumerate(current_pages, 1):
        ...
        # バックアップページのプロパティを取得
        backup_properties = backup_page.get("properties", {})
        
        # 復元するプロパティを準備
        properties_to_update = {}
        
        # 各プロパティを変換
        for prop_name, prop_data in backup_properties.items():
            if prop_name in ["名前", "最終更新日時"]:  # これらのプロパティは更新しない
                continue
                
            converted_prop = convert_property_for_update(prop_data)
            if converted_prop:
                properties_to_update[prop_name] = converted_prop
        
        # プロパティが存在する場合のみ更新
        if properties_to_update:
            page_id = page.get("id")
            if update_page_properties(page_id, properties_to_update):
                print(f"  プロパティを更新しました: {list(properties_to_update.keys())}")
                updated_count += 1
            else:
