...
# -*- coding: utf-8 -*-
"""
Fix Remaining Newlines - 改行が不適切なページを修正
"""
...
def load_newlines_check_result() -> List[Dict[str, Any]]:
    """改行チェック結果を読み込み"""
    try:
        with open("newlines_check_result.json", 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"改行チェック結果ファイル読み込みエラー: {e}")
        return []
...
def main():
    ...
    # 改行チェック結果を読み込み
    check_results = load_newlines_check_result()
    if not check_results:
        print("改行チェック結果ファイルが見つかりませんでした。")
        return
    
    # 改行が不適切なページを抽出
    improper_pages = [result for result in check_results if not result["has_proper_newlines"]]
    
    print(f"修正対象ページ数: {len(improper_pages)}")
    print()
    ...
    for i, page_result in enumerate(improper_pages, 1):
        title = page_result["title"]
        
        print(f"[{i}/{len(improper_pages)}] 修正中: {title}")
        
        # バックアップデータから該当ページを取得
        if title not in backup_dict:
            print(f"  バックアップデータにページが見つかりませんでした")
            failed_count += 1
            continue
        
        page_data = backup_dict[title]
        ...
        # 進捗表示（10ページごと）
        if i % 10 == 0:
            print(f"  進捗: {i}/{len(improper_pages)} ページ完了")
