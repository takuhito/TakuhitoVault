...
# -*- coding: utf-8 -*-
"""
Bulk Recreate Pages - 全ページ一括再作成ツール
様々なゴミ文字パターンに対応し、改行を保持して全ページを再作成します。
"""
...
def create_clean_page(title: str, content: str, properties: Dict[str, Any]) -> str:
    ...
    try:
        ...
        # タイトル
        if "名前" in properties:
            page_properties["名前"] = properties["名前"]
        
        # URL（チャットID）
        if "URL" in properties:
            page_properties["URL"] = properties["URL"]
        
        # 最終更新日時
        if "最終更新日時" in properties:
            page_properties["最終更新日時"] = properties["最終更新日時"]
        ...
        # AI Model
        if "AI Model" in properties:
            page_properties["AI Model"] = properties["AI Model"]
