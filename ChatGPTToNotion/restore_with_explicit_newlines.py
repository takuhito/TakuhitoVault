...
# -*- coding: utf-8 -*-
"""
Restore With Explicit Newlines - 明示的な改行処理で復元
改行を明示的に処理してページを復元します。
"""
...
def create_page_with_explicit_newlines(title: str, content: str) -> str:
    """明示的な改行処理でページを作成"""
    try:
        # 改行を明示的に処理
        lines = content.split('\n')
...
def main():
    ...
    for i, page_data in enumerate(test_pages, 1):
        ...
        # 新しいページを作成（明示的な改行処理）
        new_page_id = create_page_with_explicit_newlines(title, cleaned_content)
        if new_page_id:
