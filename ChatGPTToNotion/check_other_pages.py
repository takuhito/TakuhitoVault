    ...
    for i, block in enumerate(blocks):
        ...
        if block_type == "paragraph":
            ...
            if rich_text:
                ...
                found_garbage = [char for char in garbage_chars if char in content]
                if found_garbage:
                    print(f"    ⚠️  ゴミ文字発見: {found_garbage}")
                    garbage_found = True
...
def main():
    ...
    # 確認するページのタイトル
    check_titles = ["データに基づく提案", "Conversation Summary", "こんにちは たくひと", "R-1グランプリのR"]
    ...
    for title in check_titles:
        ...
        check_page_content(page_id, title)
        print(f"{'='*60}")
